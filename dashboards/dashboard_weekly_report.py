...
def prepare_report_data(report_week, season, cache_dir, output_filename="report_card_data.json"):
    ...
    accolade_counts_file = Path("accolade_counts.json")
    ...
    logging.info(f"✅ Successfully saved report card data to: {output_filename}")
...
def main():
    ...
    logging.info(f"Currently in week {current_week} of the {TARGET_SEASON} season.")
    
    last_completed_week = current_week - 1
    if last_completed_week == 0:
        logging.info("No weeks have been completed yet.")
        return
    ...
    # --- Generate Report ---
    prepare_report_data(last_completed_week, TARGET_SEASON, cache_dir)
